...
function setupEventListeners() {
  ...
  $("#movieSelect, #dateSelect, #timeSelect, #ticketCount").change(
...
function updateBookingSummary() {
  ...
  if (!movieId) {
    $("#bookingSummary").html(
      '<p class="text-muted">Select movie and show details to see summary</p>'
    );
    return;
  }
  ...
  if (ticketCount) {
    ...
    html += `
            <hr>
            <p>Tickets: ${ticketCount} × $${movie.price} = $${subtotal.toFixed(
      2
    )}</p>
...
function handleSeatSelection() {
  ...
  if ($(this).hasClass("selected")) {
    ...
    if (selectedSeats.length < ticketCount) {
      ...
      showNotification(
        "You have already selected the maximum number of seats",
        "warning"
      );
      return;
    }
...
function createBookingItem(booking) {
  ...
  if (booking.food && booking.food.length > 0) {
    foodItems =
      '<br><small class="text-muted">Food: ' +
  ...
  return `
        <div class="booking-item">
            ...
            <p class="mb-1">
                ...
                <strong>Date:</strong> ${formatDate(booking.date)} at ${
    booking.time
  }<br>
